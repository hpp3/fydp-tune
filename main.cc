...
double reduce(int bitsize) {
    ...
    auto id = tuner.AddKernelFromString(full_kernel, "reduction_sum_scalar_complete2", {(unsigned long)simpleSize}, {(unsigned long)group_size});

    tuner.AddArgumentInput(data);
    tuner.AddArgumentInput(sum);

    tuner.SetNumRuns(1000);
    tuner.Tune();

    tuner.PrintJSON("fydp-tune.json", {});
...
void easy_reduce() {
    ...
    PROCESS_VEC(data, "test", false, group_size);

    std::string kernel = KERNEL_STRING("../samples/fydp-tune/reduce.opencl");

    cltune::Tuner tuner(size_t{1}, size_t{0});

    auto id = tuner.AddKernelFromString(kernel, "reduction_sum_scalar_complete2", {(unsigned long)simpleSize}, {(unsigned long)group_size});
